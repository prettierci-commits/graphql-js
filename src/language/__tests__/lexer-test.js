...
`)
  ...
  it("lexes strings", () => {
    expect(lexOne('"simple"')).to.deep.equal({
    ...
    expect(lexOne('" white space "')).to.deep.equal({
      kind: TokenKind.STRING,
      start: 0,
      end: 15,
      value: " white space "
    ...
    expect(lexOne('"quote \\""')).to.deep.equal({
      kind: TokenKind.STRING,
      start: 0,
      end: 10,
      value: 'quote "'
    ...
    expect(lexOne('"slashes \\\\ \\/"')).to.deep.equal({
      kind: TokenKind.STRING,
      start: 0,
      end: 15,
      value: "slashes \\ /"
    ...
    expect(lexOne('"unicode \\u1234\\u5678\\u90AB\\uCDEF"')).to.deep.equal({
      kind: TokenKind.STRING,
      start: 0,
      end: 34,
      value: "unicode \u1234\u5678\u90AB\uCDEF"
  ...
  it("lex reports useful string errors", () => {
    ...
    expect(lexErr('"no end quote')).to.throw(
      "Syntax Error GraphQL (1:14) Unterminated string"
    );

    expect(lexErr('"contains unescaped \u0007 control char"')).to.throw(
      'Syntax Error GraphQL (1:21) Invalid character within String: "\\u0007".'
    );

    expect(lexErr('"null-byte is not \u0000 end of file"')).to.throw(
  ...
  it("lexes numbers", () => {
    ...
    expect(lexOne("4.123")).to.deep.equal({
    ...
    expect(lexOne("0.123")).to.deep.equal({
      kind: TokenKind.FLOAT,
      start: 0,
      end: 5,
      value: "0.123"
    ...
    expect(lexOne("123e4")).to.deep.equal({
      kind: TokenKind.FLOAT,
      start: 0,
      end: 5,
      value: "123e4"
    ...
    expect(lexOne("123e-4")).to.deep.equal({
      kind: TokenKind.FLOAT,
      start: 0,
      end: 6,
      value: "123e-4"
    ...
    expect(lexOne("-1.123e4")).to.deep.equal({
      kind: TokenKind.FLOAT,
      start: 0,
      end: 8,
      value: "-1.123e4"
    ...
    expect(lexOne("-1.123e-4")).to.deep.equal({
      kind: TokenKind.FLOAT,
      start: 0,
      end: 9,
      value: "-1.123e-4"
    ...
    expect(lexOne("-1.123e4567")).to.deep.equal({
      kind: TokenKind.FLOAT,
      start: 0,
      end: 11,
      value: "-1.123e4567"
  ...
  it("lex reports useful number errors", () => {
    ...
    expect(lexErr(".123")).to.throw(
      'Syntax Error GraphQL (1:1) Unexpected character "."'
    );

    expect(lexErr("1.A")).to.throw(
      "Syntax Error GraphQL (1:3) Invalid number, " +
        'expected digit but got: "A".'
    );

    expect(lexErr("-A")).to.throw(
      "Syntax Error GraphQL (1:2) Invalid number, " +
        'expected digit but got: "A".'
    );

    expect(lexErr("1.0e")).to.throw(
      "Syntax Error GraphQL (1:5) Invalid number, " +
        "expected digit but got: <EOF>."
    );

    expect(lexErr("1.0eA")).to.throw(
      "Syntax Error GraphQL (1:5) Invalid number, " +
        'expected digit but got: "A".'
    );
  });
