...
describe("astFromValue", () => {
  ...
  it("converts numeric values to ASTs", () => {
    expect(astFromValue(123)).to.deep.equal({ kind: "IntValue", value: "123" });
  ...
  it("converts array values to List ASTs", () => {
    ...
    expect(astFromValue(["FOO", "BAR"], new GraphQLList(myEnum))).to.deep.equal(
      {
        kind: "ListValue",
        values: [
  ...
  it("converts list singletons", () => {
    expect(astFromValue("FOO", new GraphQLList(myEnum))).to.deep.equal({
      kind: "EnumValue",
      value: "FOO"
    });
  });
